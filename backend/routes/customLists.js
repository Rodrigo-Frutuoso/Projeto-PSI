...
router.get('/', authMiddleware, async (req, res) => {
    ...
    // GET /api/custom-lists/:id — Obter uma lista personalizada com o seu conteúdo
    router.get('/:id', authMiddleware, async (req, res) => {
        try {
            const list = await CustomList.findOne({ _id: req.params.id, user: req.userId })
                .populate({
                    path: 'albums.album',
                    select: 'title releaseYear artista',
                    populate: {
                        path: 'artista',
                        select: 'name'
                    }
            ...
            console.error('Erro ao obter lista personalizada:', error);
            res.status(500).json({ message: 'Erro ao obter a lista personalizada.' });
        }
    });
    ...
    router.delete('/:id', authMiddleware, async (req, res) => {
        try {
            const deletedList = await CustomList.findOneAndDelete({ _id: req.params.id, user: req.userId });

            if (!deletedList) {
                return res.status(404).json({ message: 'Lista personalizada não encontrada.' });
            }

            res.status(200).json({ message: 'Lista personalizada removida com sucesso!' });

        } catch (error) {
            console.error('Erro ao remover lista personalizada:', error);
            res.status(500).json({ message: 'Erro ao remover a lista personalizada.' });
        }
    });
